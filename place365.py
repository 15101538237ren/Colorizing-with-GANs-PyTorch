...
parser.add_argument('--dataset', type=str, default='place365', help='the name of dataset [places365, cifar10] (default: place365)')
...
parser.add_argument("--plot_batch_size", type=int, default=4, help="size of the batches in plotting")
parser.add_argument("--device", type=str, default='cuda', help="whether the algorithm executed on cpu or gpu")
parser.add_argument("--gpu-id", type=int, default=4, help="the gpu id for running")
...
class Generator(nn.Module):
    def __init__(self, name, output_channels=3, training=True):
        ...
        self.encoder_block_7 = GeneratorEncoderBlock(512, 512, 2)
        self.encoder_block_8 = GeneratorEncoderBlock(512, 512, 2)

        self.decoder_block_1 = GeneratorDecoderBlock(512, 512, 2)
        self.decoder_block_2 = GeneratorDecoderBlock(512, 512, 2)
        self.decoder_block_3 = GeneratorDecoderBlock(512, 512, 2)
        self.decoder_block_4 = GeneratorDecoderBlock(512, 256, 2)
        self.decoder_block_5 = GeneratorDecoderBlock(256, 128, 2)
        self.decoder_block_6 = GeneratorDecoderBlock(128, 64, 2)
        self.decoder_block_7 = GeneratorDecoderBlock(64, 64, 2)
    ...
    def forward(self, inputs):
        ...
        layers.append(output)
        output = self.encoder_block_7(output)
        layers.append(output)
        output = self.encoder_block_8(output)
        layers.append(output)
        ...
        output = self.decoder_block_6(output)
        output = output + layers[len(layers) - 5 - 2]

        output = self.decoder_block_7(output)
        output = output + layers[len(layers) - 6 - 2]
...
class Discriminator(nn.Module):
    def __init__(self, name, training=True):
        ...
        self.discriminator_block_5 = GeneratorEncoderBlock(512, 512, 2)
        self.discriminator_block_6 = GeneratorEncoderBlock(512, 512, 2)
...
if training:
    ...
    training_data_dir = os.path.join(dataset_dir, "data_256")
    trainset_color = torchvision.datasets.ImageFolder(root=training_data_dir, transform=loading_transform)
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (real_color_imgs,_) in tqdm.tqdm(enumerate(trainloader_color)):
            ...
            if cuda:
                valid = valid.cuda(gpu_id)
                fake = fake.cuda(gpu_id)
                gray_scale_imgs = gray_scale_imgs.cuda(gpu_id)
                real_color_imgs = real_color_imgs.cuda(gpu_id)
            # -----------------
            #  Train Generator
            # -----------------
    ...
    testing_data_dir = os.path.join(dataset_dir, "data_256")
    testset = torchvision.datasets.ImageFolder(root=testing_data_dir, transform=loading_transform)
